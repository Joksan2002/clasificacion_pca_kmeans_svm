...
st.title("Clasificador MNIST con Reducción de Dimensionalidad (PCA, K-Means & SVM) - Inteligencia Artificial - Joksan Zavala - 20201900395")
st.markdown("""
...
# Validar que los modelos se cargaron
if pca_model is None or svm_model is None:
...
# Requisito: Seleccionar el número de componentes principales (PCA)
...
st.sidebar.markdown("---")
st.sidebar.header("Probar con una Imagen de Muestra")

# Cargamos el dataset original (o una pequeña fracción) para simular la predicción de una imagen
@st.cache_data
...
if X_raw is not None:
    idx_imagen = st.sidebar.slider("Selecciona un índice de imagen (0-99)", 0, 99, 10)
    imagen_seleccionada = X_raw[idx_imagen]
    etiqueta_real = y_raw[idx_imagen]
else:
    # Generar un dígito por defecto aleatorio si no encuentra el archivo csv localmente
    st.sidebar.warning("Para probar imágenes en tiempo real, asegúrate de incluir el archivo `train.csv` en el directorio de la app.")
...
with col_img:
    ...
    # Remodelar el vector plano de 784 píxeles a una matriz de 28x28 para graficar la imagen
...
with col_pred:
    ...
    # 1. Normalizar y preparar la entrada
    imagen_normalizada = imagen_seleccionada / 255.0 if imagen_seleccionada.max() > 1.0 else imagen_seleccionada
    ...
    # Para predecir con SVM y K-Means que esperan el número fijo con el que se entrenaron,
    # rellenamos con ceros si el slider es menor, asegurando estabilidad:
    ...
    # Breve explicación requerida
    st.markdown(f"""
    **Explicación del Resultado:** La imagen original de 784 dimensiones (28x28 píxeles) se comprimió usando los componentes principales de PCA. 
